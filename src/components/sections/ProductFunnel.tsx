                    ...
                    <div className="inline-flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shadow-inner">
                        {siteData.funnel.steps.map((step) => (
                            <button
                                ...
                                className={`relative px-8 py-3 rounded-xl font-bold transition-all duration-300 ${activeTab === step.id ? 'text-primary' : 'text-slate-500 hover:text-slate-700'
                                    }`}
                            >
                                {activeTab === step.id && (
                                    <motion.div
                        ...
                        >
                            ...
                            <div className="order-1 lg:order-2">
                                <div className="relative">
                                    {/* Mockup Frame */}
                                    <div className="glass-card p-4 rounded-3xl relative overflow-hidden aspect-[4/3] flex items-center justify-center bg-slate-50 border-4 border-white shadow-2xl">
                                        <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-transparent" />
                                        {/* Fake Project Image Replacement */}
                                        <div className="text-center p-8">
                                            <div className="w-20 h-20 bg-primary/20 rounded-2xl flex items-center justify-center text-primary mx-auto mb-4">
                                                <ArrowRight className="w-10 h-10 rotate-45" />
                                            </div>
                                            <p className="font-bold text-slate-400 uppercase tracking-widest text-sm mb-2">Visualização de Interface</p>
                                            <p className="text-slate-600 font-medium">Dashboard de {activeStep.label} em tempo real.</p>
                                        </div>
                                    </div>
                                    {/* Floating Elements */}
